...
frozen_epochs=0 # for the first frozen_epochs epochs, freeze the actor
...
actor_max_token_len_per_gpu=12288 # If using dynamic batch size, the following 3 configs is the max token length per gpu for actor rollout.
...
######### Names and Paths ##########
# project_name='ai_adult_income_evolve'
# project_name='ai_bank_marketing_evolve'
# project_name='ai_boston_housing_evolve'
project_name="debug"
...
train_datapath=$(realpath ../../data)/datasets/verl/ai_coding/train.parquet
test_datapath=$(realpath ../../data)/datasets/verl/ai_coding/test.parquet
...
nsga_knn_avg=5
nsga_embed_model=Salesforce/codet5p-110m-embedding
...
PYTHONPATH=$python_path python3 -m recipe.algo_evolve.main_evolve \
    ...
    data.custom_cls.path=./dataset/dynamic_rl_dataset.py \
    data.custom_cls.name=DynamicRLHFDataset \
    +data.custom_cls.init_kwargs.log_path=$project_base_path/evolve_dataset_logs \
    +data.custom_cls.init_kwargs.language=$language \
    ...
    +data.dynamic_sample_strategy.nsga_config.knn_avg=$nsga_knn_avg \
    +data.dynamic_sample_strategy.nsga_config.embed_model=$nsga_embed_model \
    ...
    trainer.critic_warmup=$frozen_epochs \
